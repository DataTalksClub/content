...
def _post_correction_bytes() -> bytes:
    """The article content immediately after this correction landed, addressed
    by its immutable git blob id. The live article has since gained further,
    independently declared edits (front-matter FAQ pairs), so this narrow
    correction's own pinned target is fetched by content address rather than
    read from the current working tree.
    """
    result = subprocess.run(
        ["git", "cat-file", "-p", source_correction.ARTICLE_TARGET_BLOB],
        cwd=ROOT,
        check=True,
        capture_output=True,
    )
    assert hashlib.sha256(result.stdout).hexdigest() == source_correction.ARTICLE_TARGET_SHA256
    return result.stdout


def _legacy_article_bytes() -> bytes:
    current = _post_correction_bytes().decode("utf-8")
...
def _copy_contract_targets(destination: Path) -> None:
    for relative in (
        source_correction.MIGRATION_MANIFEST_RELATIVE_PATH,
...
def test_article_correction_replaces_complete_destinations_only() -> None:
    ...
    assert corrected == _post_correction_bytes()
...
def test_article_overlay_rejects_reverted_correction(tmp_path: Path) -> None:
    candidate = tmp_path / "repository"
    _copy_contract_targets(candidate)
    article = candidate / ARTICLE_PATH
    reverted = article.read_text(encoding="utf-8").replace(
        "https://aishippingblog.com/p/ai-native-development-specifications",
        "https://alexeyondata.substack.com/p/ai-native-development-specifications",
    )
    article.write_text(reverted, encoding="utf-8")

    with pytest.raises(SourceCorrectionError, match="legacy destination still present"):
        validate_source_correction(candidate)


def test_article_overlay_tolerates_unrelated_later_drift(tmp_path: Path) -> None:
    # The live article can legitimately gain further, independently declared
    # edits after this correction (for example front-matter FAQ pairs); this
    # contract only owns the five link destinations, so unrelated later
    # drift must not trip it.
    candidate = tmp_path / "repository"
    _copy_contract_targets(candidate)
    article = candidate / ARTICLE_PATH
...
def test_manifest_rejects_podcast_target_byte_drift(tmp_path: Path) -> None:
    candidate = tmp_path / "repository"
    _copy_contract_targets(candidate)
    podcast = candidate / PODCAST_PATH
    podcast.write_bytes(podcast.read_bytes() + b"\nundeclared: true\n")

    with pytest.raises(SourceCorrectionError, match="podcast 1: target digest differs"):
        validate_source_correction(candidate)
