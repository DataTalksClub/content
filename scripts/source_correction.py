...
MANIFEST_RELATIVE_PATH = Path("migration/editorial-overlays/2026-09-01-source-corrections.yaml")
EXPECTED_MANIFEST_SHA256 = "b1511aab3e170c26aab0f944ba3fa5a6d4fa57b66656347bec5d5b3866cc83a5"
...
MIGRATION_MANIFEST_RELATIVE_PATH = "migration/migration.yaml"
MIGRATION_MANIFEST_SHA256 = "dd78a343a5f387a74afa914fc6c7e19790e202aa5d6fa9aba08bfda5995c5f86"

# Current, season-hierarchical target paths this overlay validates against the
# live tree. The correction commit itself (``e06e138``) touched these files at
# their pre-reorg flat locations; that historical shape is preserved verbatim
# in ``PODCAST_HISTORICAL_PATH`` and ``EXPECTED_CHANGED_PATHS`` below.
ARTICLE_PATH = "articles/2025/25-09-23-ai-dev-tools-zoomcamp.md"
...
PODCAST_PATH = "podcasts/s24/e06.yaml"
PODCAST_HISTORICAL_PATH = "podcasts/s24e06-how-to-build-ai-that-actually-ships-in-production.yaml"
...
PODCAST_TARGET_BLOB = "f5eec75847196ea67005b584ce9c415506ff64c9"
PODCAST_TARGET_SHA256 = "548d70c1c2129abafa2c8e4718c05f67c4ee7641be973a6af45e4a60427a1295"
PODCAST_FIELD = "image"
...
def _validate_article_row(root: Path, row: Any) -> dict[str, Any]:
    ...
    # ``target_blob``/``target_sha256`` above pin the byte-identical result of
    # applying only these five replacements to the immutable legacy source;
    # they document that narrow transform and are not re-checked against the
    # live file here. The live article is free to gain later, independently
    # declared edits (for example added front matter), so this only fails
    # closed on the one thing this contract owns: the legacy destinations
    # must be gone and the corrected destinations must remain present.
    target = _regular_file(root, ARTICLE_PATH, prefix)
    try:
        target_text = target.read_text(encoding="utf-8")
    except UnicodeDecodeError as error:
        raise SourceCorrectionError(f"{prefix}: target is not valid UTF-8") from error
    for replacement in normalized:
        old_matches = _link_matches(target_text, replacement["match"], replacement["old"])
        _expect(
            not old_matches,
            f"{prefix}: legacy destination still present in live target",
        )
        new_matches = _link_matches(target_text, replacement["match"], replacement["new"])
        _expect(
            len(new_matches) >= replacement["occurrences"],
            f"{prefix}: corrected destination missing from live target",
        )
    return row
